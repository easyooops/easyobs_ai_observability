"""S3 Parquet blob store.

Writes trace spans as Parquet files to Amazon S3 using boto3 with
hive-style partitioning. DuckDB's httpfs extension reads these files
directly via ``s3://bucket/prefix/**/*.parquet`` scan URIs.

Credentials: when access_key_id / secret_access_key are empty, boto3
falls through its default credential chain (env vars, ~/.aws/credentials,
IAM instance role / IRSA).
"""
...
class S3ParquetBlobStore:
    """Parquet blob store backed by Amazon S3."""

    def __init__(self, cfg: BlobConfig) -> None:
        self._bucket = cfg.bucket
        self._prefix = (cfg.prefix or "traces").strip("/")
        self._region = cfg.region or None
        self._access_key_id = cfg.s3_access_key_id or None
        self._secret_access_key = cfg.s3_secret_access_key or None
    ...
    def _object_key(self, relpath: str) -> str:
    ...
    def read_batch_lines(self, batch_relpath: str) -> list[dict[str, Any]]:
        """Download and read a single Parquet (or legacy NDJSON) batch from S3."""
        key = self._object_key(batch_relpath)
        try:
        ...
        if batch_relpath.endswith(".parquet"):
            return self._read_parquet_bytes(body)
        return self._read_ndjson_bytes(body)
    ...
    def scan_uri(self, pattern: str = "**/*.parquet") -> str:
        return f"s3://{self._bucket}/{self._prefix}/{pattern}"
