...
프로젝트 **루트**에서 아래 명령을 실행합니다. EasyObs 를 운영용 AWS 환경에
한 번에 배포할 수 있도록 두 가지 토폴로지를 제공합니다.

| 토폴로지 | 용도 | Terraform 디렉터리 |
|----------|------|----------------------|
| **single** | 단일 EC2 1대에 API + Web + Postgres compose 풀스택 | `setup/ec2/terraform/single/` |
| **cluster** | ALB + API leader EC2 + API worker ASG + Web EC2 + RDS Postgres + EFS (수평 확장) | `setup/ec2/terraform/cluster/` |
...
- 프로젝트 루트 체크아웃 — Terraform 이 EasyObs 소스 트리를
...
- 빌드 스크립트: `setup/offline/build-bundle.sh`
  / `build-bundle.ps1`
- 가이드 문서:   `setup/offline/README.md`
...
| 항목 | 경로 |
|------|------|
| EasyObs 소스 (개발용) | 프로젝트 루트 |
| Dockerfile (API)      | `setup/images/api/Dockerfile` |
| Dockerfile (Web)      | `setup/images/web/Dockerfile` |
| Compose (단일/클러스터) | `setup/compose/` |
| Terraform (single)    | `setup/ec2/terraform/single/` |
| Terraform (cluster)   | `setup/ec2/terraform/cluster/` |
| 오프라인 번들 도구     | `setup/offline/` |
| 종합 README           | `setup/README.md` |
