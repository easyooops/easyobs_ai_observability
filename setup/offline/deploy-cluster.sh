    ...
    case "$ROLE" in
      leader|worker)
        ...
        docker run -d \
          ...
          -e EASYOBS_STORAGE_FORMAT="${EASYOBS_STORAGE_FORMAT:-parquet}" \
          -e EASYOBS_QUERY_ENGINE="${EASYOBS_QUERY_ENGINE:-duckdb}" \
          ...
          ${AZURE_OPENAI_API_KEY:+-e AZURE_OPENAI_API_KEY="$AZURE_OPENAI_API_KEY"} \
