...
`docs/comparison/02.design` 의 **OpenAPI · SQLite(또는 Postgres) · 객체-우선(로컬 파일 / S3·Azure·GCS) · OTLP/HTTP** 방향을 따른 EasyObs MVP 입니다.
...
> 모든 명령은 저장소 안의 `docs/comparison/03.develop/easyobs/` 디렉터리에서
> 실행하는 것을 기준으로 합니다.

### 1-1. Windows (PowerShell)

```powershell
# 저장소 루트에서 EasyObs 모듈 폴더로 이동
Set-Location docs\comparison\03.develop\easyobs
...
`docs/comparison/02.design/05~08` 의 평가 설계를 구현한 모듈입니다. 운영 트레이스
...
설계 문서: [`02.design/05~08`](../../02.design)
...
- **AI Security Hardening 트랙 (Mythos 위협 대응)**: 2026년 4월 Anthropic
  ...
  (`rule.safety.injection_pattern` 등)를 사전 와이어링했습니다. 위협 모델·
  운영 절차·Threshold 알림 시그널은 `docs/comparison/02.design/10.ai-security-hardening.md`
  를 참조하세요. 룰 본체 구현은 후속 PR이며, 그 전에도 Mythos급 사고 발생
